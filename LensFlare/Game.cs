...
namespace LensFlare
{
    ...
    public class LensFlareGame : Game
    {
        ...
        Color dayTime = Color.CornflowerBlue;
        SpriteFont font;
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            SetGlowLevel(gameTime);

            HideQueryPolygon();
        }

        public void SetGlowLevel(GameTime gameTime)
        {
            float time = (float)gameTime.ElapsedGameTime.TotalMilliseconds;

            if (currentKeyboardState.IsKeyDown(Keys.B))
            {
                lensFlare.glowSize += 10 * time * 0.01f;
            }
        ...
        public void HideQueryPolygon()
        {
            if (currentKeyboardState.IsKeyDown(Keys.X))
            {
                lensFlare.hideQueryPolygon = true;
            }
            else if (currentKeyboardState.IsKeyDown(Keys.C))
            {
                lensFlare.hideQueryPolygon = false;
            }
        }

        public void DisplayInfoBrightnessVisibility(string label, GameTime gameTime, int x, int y)
        {
            Color textColor = Color.White;
            Vector2 labelPosition = new Vector2(x, y);

            spriteBatch.DrawString(font, label, labelPosition, textColor);
        }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            Matrix projection = Matrix.CreatePerspectiveFieldOfView(MathHelper.PiOver4, aspectRatio, 0.1f, 500);
            ...
            if (lensFlare.hideQueryPolygon)
            {
                float visibilityPerCent = lensFlare.occlusionAlpha * 100;
                float occlusionPerCent = visibilityPerCent - 100;

                spriteBatch.Begin();
                DisplayInfoBrightnessVisibility("Brightness: " + (int)lensFlare.glowSize, gameTime, 100, 100);
                DisplayInfoBrightnessVisibility("Visibility: " + visibilityPerCent + "%", gameTime, 100, 140);
                DisplayInfoBrightnessVisibility("Occlusion: " + -occlusionPerCent + "%", gameTime, 100, 180);
                spriteBatch.End();
            }            
        ...
        private void UpdateCamera(GameTime gameTime)
        {
            ...
            Vector3 tiltedFront = Vector3.TransformNormal(cameraFront, pitchMatrix * turnMatrix);
            ...
            cameraPosition += cameraFront * currentGamePadState.ThumbSticks.Left.Y * time * 0.1f;
